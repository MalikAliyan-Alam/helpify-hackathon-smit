...
export function PollsPage() {
  ...
  return (
    <div className="flex flex-col gap-8 pb-12">
      {/* Header */}
      <div className="bg-[#2b3231] rounded-[24px] p-10 flex flex-col md:flex-row md:items-center justify-between text-white gap-6">
        <div className="flex-1">
          <p className="text-gray-400 font-bold text-xs uppercase tracking-wider mb-4">COMMUNITY / GOVERNANCE</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-8 items-start">
        ...
        <div className="space-y-6">
          {loading ? (
            ...
            <Card className="p-20 text-center bg-white/50 border-dashed border-gray-200">
               <p className="text-gray-400 italic">No active polls. Be the first to start a discussion!</p>
            </Card>
          )}
        </div>
        ...
        <div className="space-y-6">
          {showForm && (
             <Card className="p-8 border-none shadow-2xl bg-white sticky top-24 animate-in slide-in-from-right duration-500">
                <p className="text-[#129780] font-black text-[10px] uppercase tracking-widest mb-4">POLL CONFIGURATION</p>
                <form onSubmit={handleCreatePoll} className="space-y-6">
                   <div>
                      ...
                      <textarea 
                        ...
                        className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#129780] resize-none"
                      />
                   </div>
                   <div className="space-y-3">
                      ...
                      {options.map((opt, i) => (
                        <input 
                          ...
                          className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#129780]"
                        />
                      ))}
                      {options.length < 4 && (
                        <button 
                          ...
                          className="w-full py-2 border border-dashed border-gray-200 rounded-xl text-[10px] font-bold text-gray-400 hover:text-[#129780] hover:border-[#129780]/30 transition-all"
                        >
                          + ADD OPTION
                        </button>
                      )}
