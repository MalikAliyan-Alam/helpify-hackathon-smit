...
export function StreakWidget({ currentStreak = 0, longestStreak = 0, freezes = 0 }) {
  ...
  return (
    <Card className="border-none shadow-sm rounded-[24px] p-6 lg:p-8 flex flex-col relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <p className="text-[var(--accent)] font-bold text-[10px] uppercase tracking-wider mb-1">MOMENTUM</p>
          <h3 className="text-xl font-bold text-[var(--text-primary)]">Helping Streak</h3>
        </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex justify-between items-center">
          <div className="space-y-0.5">
            <p className="text-[10px] text-[var(--text-secondary)] font-bold uppercase tracking-widest mb-1">Current Streak</p>
            <div className="flex items-baseline gap-1.5">
              <span className="text-5xl font-black text-[var(--text-primary)] tracking-tighter">{currentStreak}</span>
              <span className="text-sm font-bold text-[var(--text-secondary)]">days</span>
            </div>
          </div>
          <div className="text-right flex flex-col items-end">
            <p className="text-[9px] text-[var(--text-secondary)] font-bold uppercase tracking-widest mb-1">Best Ever</p>
            <p className="text-2xl font-black text-[var(--accent)]">{longestStreak}</p>
          </div>
        </div>
        ...
        <div className="space-y-2.5">
          <div className="flex justify-between text-[9px] font-bold uppercase tracking-[0.15em]">
            ...
            <span className="text-[var(--accent)]">{currentStreak >= 7 ? '30 Days' : '7 Days'}</span>
          </div>
          <div className="w-full h-3 bg-[var(--bg-secondary)] rounded-full overflow-hidden border border-[var(--border-color)]">
            <div 
              className="h-full bg-gradient-to-r from-[var(--accent)] to-[#0f7a68] transition-all duration-1000"
        ...
        <div className="pt-6 border-t border-[var(--border-color)]">
          <p className="text-[9px] text-[var(--text-secondary)] font-bold uppercase tracking-[0.15em] mb-4">Activity Log</p>
          <div className="flex flex-col gap-2">
            <div className="flex gap-2.5">
              {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, i) => (
                <span key={i} className="flex-1 text-center text-[9px] font-bold text-[var(--text-secondary)]">{day}</span>
              ))}
            </div>
            <div className="flex gap-2.5">
              {activity.map((active, i) => (
                <div 
                  key={i} 
                  className={`flex-1 h-12 rounded-xl transition-all flex items-center justify-center ${active ? 'bg-[var(--accent)] shadow-sm shadow-[var(--accent)]/10' : 'bg-[var(--bg-secondary)] border border-[var(--border-color)]'}`}
                >
                  {active && (
